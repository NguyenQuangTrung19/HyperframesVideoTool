...
| `PODCAST_LAYOUT` | `fullbleed` | Portrait-source layout preset (ignored when source is landscape — that always uses the dedicated `landscape` layout). **`fullbleed`** (default) — aesthetic-bg mode: source footage fills the entire 1080×1920 canvas (cover crop) with optional dim overlay, NO card. Corner badge top-left: rounded logo tile (assets/logoPodcast.png) + "Life Podcast" Palatino italic beside it (lockup, added 2026-06-11); karaoke captions overlay as lower-third. Use when the source is high-quality scenery / street / ambient footage where the image quality IS the draw. **`card`** (legacy) — solid black canvas + 880×880 rounded card centered + thin white outline + centered wordmark above. **`vignette`** (legacy) — viral aesthetic: blurred-source bg + sharp card + search bar + progress bar + watermark. **Landscape layout** (auto-selected for 16:9 sources): solid black canvas, 16:9 strip scaled to width=1080 centered vertically (small upward bias), same corner badge + "Life Podcast" lockup top-left, karaoke captions burned INSIDE the strip's lower portion with 80 px safe margins — no card chrome |
